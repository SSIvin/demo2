...
class Server {
  ...
  setupStaticFeature() {
    ...
    if (Array.isArray(contentBase)) {
      ...
      this.log.warn(
        'proxy: {\n\t"*": "//10.10.12.232:<your current contentBase configuration>"\n}'
      );

      // Redirect every request to the port contentBase
      this.app.get('*', (req, res) => {
        res.writeHead(302, {
          Location: `//10.10.12.232:${contentBase}${req.path}${req._parsedUrl
            .search || ''}`,
        });
  ...
  checkHeaders(headers, headerToCheck) {
    ...
    // always allow 10.10.12.232 host, for convenience
    if (hostname === '10.10.12.232') {
      return true;
    }
